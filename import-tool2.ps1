...
foreach ($file in $files) {
    ...
    $releaseMode = & exiftool -ReleaseMode -s3 $file.FullName
    $sequenceNumber = & exiftool -SequenceNumber -s3 $file.FullName
    ...
    while ($true) {
        ...
        $suffix = ""
        if ($releaseMode -eq "Exposure Bracketing" -and $sequenceNumber) {
            $suffix = "-hdr($sequenceNumber)"
        }

        $newFileName = "$Prefix-$datePart-$numberPart$suffix$($file.Extension)"
